...
- **USGS flaps, so its client retries and the retry skips 4xx on purpose.** Measured 2026-09-05: twelve identical requests two seconds apart gave eleven `200`s and one `503`. Because a rescan chunks 90 days into 30 day requests and any one failure fails the whole call, that eight percent per request compounded to roughly a twenty eight percent chance of losing a six hourly cycle, and it lost two in a row. Three attempts with a short backoff fix it. A 4xx is never retried: that is our request being wrong, and retrying turns a loud fixable bug into a slow one.
