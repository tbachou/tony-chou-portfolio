...
/**
 * The disclaimer, carried inside every results file rather than only in the
 * directory's README. A results file is reachable on its own — a search hit, a
 * deep link, a grep of a public repo — and the first-person text inside it is
 * model output under test, some of it deliberately provoked fabrication. A
 * reader who never opens the README must still see that from the file itself.
 */
export const RESULTS_PROVENANCE =
  'Generated by the interview simulator eval suite (spec 0011). Every first-person string in this file was written by a language model under test and is NOT a claim by Tony Chou. Some cases exist to provoke overclaiming; a fabrication recorded here is a caught test failure, shown so the scoreboard can be checked. See docs/evals/interview/README.md.';

export type RunResults = {
  /** See RESULTS_PROVENANCE. First key in the file, so it is read first. */
  _readMeFirst: string;
  meta: RunMeta;
  cases: CaseResult[];
};
