...
- **`Prototype` tier, or no tier recorded** → recommend `Skip` (or `Same model` for a foundational spec).
...
2. **On Accept: ratify the decision; the status follows the spec kind** (per the status model in *What this skill does*; discriminator: whether this spec carries a `## Build plan`):
   - Buildable feature spec: do not edit the status line; a confirmed but unbuilt spec correctly stays `Proposed` (/develop advances it).
   ...
   - Already shipped documentation path: born `Accepted`; leave it.
3. **The build plan stays in the spec (this repo has no scope).** There is no `docs/scope/` here, deliberately: the spec IS the tracking surface. Its `## Build plan` holds the atomic tasks, its `## Requirements` holds the acceptance criteria, and its `**Status**:` line holds the feature's state. So there is nothing to enroll, nothing to link, and no boxes to tick. Do not offer to create a scope and do not report a missing one as a gap.

   Two things still need doing here:

   - **Check the `## Build plan` is actually buildable.** Every atomic task names the files it touches and the acceptance criteria it covers, and every value the build must produce has a named source. A task the builder would have to invent a decision for is the defect this step exists to catch, fix it in the spec now rather than letting `/develop` bounce back.
   - **Enroll what the spec surfaced.** A `## Follow-up` item that is really a separate feature (not part of this one) does not belong in this spec's Build plan. Note it in `## Follow-up` tagged `needs its own spec`, and say so in the closing summary so it can get one. Deferred follow ups that block nothing stay in `## Follow-up` as they are.

   **Decision only spec** (ARCHITECTURE stack decision or CROSS-CUTTING standard, no `## Build plan` by rule) → nothing to check here; `/develop` derives the scaffold or implementation steps from the decision at build time. Do not write those steps into the spec (the double spec bug this avoids).

4. **Spoken summary in chat (plain words, no dashes).** After acceptance, show a short plain language summary (per *Output style*): what the spec decided, why in one line, and what happens next (the build tasks it produced, and which skill to run next). A template:
