...
**Build approach** (the project's delivery strategy, read in pre-flight from root AGENTS.md `## Build approach`, or a noted default): BUILD_APPROACH
...
- Write **`## Requirements`** with the engineer's confirmed, already IDed acceptance criteria (`AC-1`, `AC-2`, …) verbatim, plus the user stories. These are the contract `/develop` builds to and `/check` checks; do not weaken or replace them. If one is genuinely missing, add it and flag it in `## Follow-up`.
