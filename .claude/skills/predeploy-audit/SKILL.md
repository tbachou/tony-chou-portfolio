...
description: Run /predeploy-audit before pushing to main. Chains the built-in security-review and code-review skills over the pending changes, adds a GitHub Actions hardening pass for workflow changes, an adversarial break-it pass, and a clinical safety auditor, then merges everything into one hold-or-clear verdict. Critical or High findings, any adversarial HIGH, or any clinical MUST-FIX mean hold the push.
---

# Pre-deploy audit gate

The gate chains two built-in skills plus three conditional auditors, then merges their findings into a single verdict. This skill never edits code, never commits, and never pushes; the deliverable is the combined report and a hold or clear verdict.
...
Steps 3 and 4 are independent and both read only, so run them concurrently in one message. Step 2b is independent of both and can join that batch.

## The gate

Merge all findings into one summary, ranked most severe first, MUST-FIX items on top. Verdict rules:

- Any Critical or High finding from either built-in review, any CRITICAL or HIGH from the Actions hardening pass, any adversarial HIGH with a confirmed failing input, or any clinical MUST-FIX: **hold the deploy**, list exactly what blocks it.
