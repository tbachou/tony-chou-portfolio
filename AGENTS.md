...
npm run check:worktrees                      # what each worktree holds, before anything expensive
...
- **worktrees: delete one the moment its branch merges.** `npm run check:worktrees` lists every worktree with its unmerged commit count, its uncommitted files, and its ignored files, which a plain `git status` never shows. It deliberately does NOT say which are safe to remove and never prints a `git worktree remove` command: three versions tried to make that call and each one cleared something irreplaceable in a new way, the last losing a hand typed `dist/.env`. Read what a directory holds, then remove it yourself. The reason any of this exists: on 2026-08-31 an eval run was started by accident in a merged worktree that still held uncommitted files, cost real budget, and produced a result no commit could reproduce. Never run anything that spends money from a worktree you have not just checked.
