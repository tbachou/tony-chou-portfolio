...
`PutEnforcedGuardrailConfiguration` designates one guardrail version that applies to all Bedrock model invocations from the account, whether or not the calling code asks for it. Verified against the AWS Guardrails enforcements documentation on 2026-08-19: the operations are `PutEnforcedGuardrailConfiguration`, `ListEnforcedGuardrailsConfiguration`, and `DeleteEnforcedGuardrailConfiguration`, it must be set in every region where enforcement is wanted, and there is an organization level equivalent through an AWS Organizations `BEDROCK_POLICY`.

**Pros**: genuinely stronger than a guardrail the application chooses to call. It cannot be bypassed by a code path that forgets it, and it survives a refactor. Guardrails bound to an enforcement configuration also cannot be deleted by default. Include and exclude model lists allow scoping.
**Cons**: **it cannot reach Beta.** The documentation is explicit that enforcement applies to `InvokeModel`, `InvokeModelWithResponseStream`, `Converse`, and `ConverseStream`, that is, invocations that go through Bedrock. Under the chosen architecture Beta's model calls go to Anthropic directly, so the clinical surface this spec exists to protect is exactly the surface account level enforcement misses. It would cover the interview simulator and the feedback classifier Lambda instead. Three further frictions: enforcement layers rather than replaces (the effective control is a union of organization, account, and request guardrails, with the most restrictive winning, and billing counts text units per guardrail ARN per request, so stacking multiplies cost); selective content guarding defaults to **Comprehensive**, meaning the enforced guardrail evaluates entire prompts including system prompts, which for Beta's drafter skill file is a large false positive surface; and the Terraform AWS provider appears not to expose the resource yet (provider issue 47400 open as of 2026-08), so it would be a console or CLI step outside Terraform, against umbrella AC-1.

**Not a substitute, possibly a complement.** Raised in Follow-up for the surfaces it does cover.
...
**Pros**: makes option 2 unforgettable at the permission layer rather than the code layer, which is the strongest form of "cannot be bypassed" available for a single account. Version pinning is expressible directly in the condition (`guardrail/<id>:1`).
...
The honest weakness of this choice is the one options 3 and 4 would have fixed: a guardrail the application chooses to call can be dropped by a future refactor. Neither of those mechanisms can reach a direct Anthropic call, so the mitigation here has to be local: the invariants below, the tests that assert the call is made, and a line in the api's `AGENTS.md` gotchas.
...
Also explicitly **not** created: an account level enforced guardrail configuration (`PutEnforcedGuardrailConfiguration`). It would not reach Beta, and enabling it would silently stack a second guardrail onto the interview simulator and the classifier Lambda, multiplying their guardrail billing and evaluating their system prompts under the Comprehensive default. See Options considered and Follow-up.
...
- [ ] Separate question, worth its own decision: should an **account level enforced guardrail configuration** be set for the surfaces it does reach, namely the interview simulator (live on Bedrock) and the feedback classifier Lambda? It cannot reach Beta, so it is out of scope here, but it is the strongest available guarantee for those two. Weigh against: no Terraform provider resource yet (provider issue 47400 open as of 2026-08), so it would be a console step against umbrella AC-1; the Comprehensive default evaluates system prompts as well as visitor text; and enforcement layers as a union, so billing counts text units per guardrail ARN per request. If taken up, it needs a policy of its own, not this clinical one.
- [ ] Re verify Sonnet 5's Bedrock availability for this account before any future decision that depends on it. The 403 is an account level fact that could change, and the provider swap child still names Sonnet 5 as its Bedrock default, which is now stale (the interview simulator is live on `us.anthropic.claude-sonnet-4-6`). That child needs a correction pass independent of this one.
...
Two stronger looking mechanisms were checked against the AWS documentation and set aside for the same reason. Account level enforcement and the `bedrock:GuardrailIdentifier` IAM condition key both make a guardrail impossible to skip, which is exactly the weakness of the chosen option. Both only reach invocations that pass through Bedrock. Beta does not, and moving it there is the trade option 2 already failed. So the unskippable versions of this control are available for every surface in this program except the one that most needs them, which is worth recording plainly rather than discovering later.
