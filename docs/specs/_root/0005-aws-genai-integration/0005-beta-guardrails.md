...
>
> **Correction, 2026-09-19.** The paragraph above was already false when it was written. Commit `e522b27` (2026-08-20) built the streaming follow up — `beta.service.ts` streams the plan rendered from the validated drafter object as soon as the drafter returns and upgrades it to the coach's prose once the guard passes — and set `BETA_OUTPUT_GUARD_MODE` to `enforce` in `render.yaml`, with AC-G9 re-run after the pre-deploy audit. Production has run at `enforce` since, confirmed 2026-09-19. The latency argument no longer applies: coach p50 measured 8.0 and 8.7 seconds across two full corpus runs on 2026-09-19. The paragraph is kept as the record of what was believed on 2026-08-22, not as guidance.
>
...
- **AC-G9** *(amended 2026-08-20 after the first corpus runs; the original "zero firings over 30 profiles" conflated three different bars and assumed every firing is the rule's fault)*: the corpus gate has three parts, each with its own pass condition.
  - **Composition** — the corpus attempts at least 30 realistic profiles across the three injury areas, including blunt injury description, profanity, and constant pain wording. Attempting a category is the requirement; whether it survives screening is measured, not assumed.
  - **Coverage** — at least 30 runs **reach the guard** (produce a plan the guard evaluates). Screened-out profiles prove nothing about the guard and do not count; the original wording was literally satisfiable by 30 profiles the screener blocked. The screener is a model and its borderline verdicts vary run to run, so a screened-out profile may be re-attempted — that is sampling a stochastic gate, not weakening it. Coverage must be reported **per required category**: a category with zero guard coverage (the first run's constant-pain profiles, all screened out) leaves its rules untested precisely where `enforce` would first meet them, and must be listed as an explicitly accepted risk if left uncovered. The gate must still run over **both** surfaces: the coach's prose *and* the drafter's plan object via `evaluatePlanContent` — a firing there is a hard error (AC-G7, `source: 'plan'`) rather than a silent fallback.
  - **Diagnosis** — every firing is diagnosed before `enforce`, not merely counted. A **false positive** (the rule fired on faithful, safe prose) blocks `enforce` until that rule is fixed: under `enforce` it would discard good output, or hard-error the request on the plan surface. A **true positive** (the output genuinely violated the skill files — e.g. the coach promising recovery against its own hard rule) does **not** block `enforce`; it is the evidence `enforce` exists to act on. It should also be fed back into the offending skill file, so the model stops producing the violation rather than relying on the guard to catch it. Zero firings with the required categories covered passes without diagnosis; the first runs produced both kinds, and a gate that cannot tell them apart either blocks forever or ships a broken rule.
...
- Live corpus run (manual, in shadow): `npm run corpus` in apps/api. At least 30 runs reaching the guard across the three injury areas, per-category coverage reported, every firing diagnosed false-positive (blocks) or true-positive (does not block, feeds the skill file). Verifies **AC-G9** as amended.
...
7. The corpus run and rule tuning. Zero UNDIAGNOSED firings is the gate on proceeding: false positives block until the rule is fixed, true positives feed the skill file and do not block. R6 is the rule most likely to need loosening. Satisfies **AC-G9** as amended.
