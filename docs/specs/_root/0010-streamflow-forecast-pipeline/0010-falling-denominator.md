...
- **AC-D4**: the allowed transition matrix for this migration is BASEFLOW stays BASEFLOW or becomes FALLING; FALLING stays FALLING; PEAK stays PEAK; RISING stays RISING; null stays null. Any other cell is a defect. Note that FALLING is now a legal source, which it was not in the first migration, and that PEAK and FALLING are both frozen.
...
| backfill | the allowed transition matrix | supplied per run rather than hardcoded. For this run: `BASEFLOW -> {BASEFLOW, FALLING}`, `FALLING -> {FALLING}`, `PEAK -> {PEAK}`, `RISING -> {RISING}`, `null -> {null}` |
...
- A backfill run whose snapshot contains a FALLING row leaves it FALLING; a run in which any PEAK or FALLING row would move fails rather than writing, verifies **AC-D4**.
...
4. Make the backfill repeatable: add the `rule` tag to the snapshot and refuse a mismatch on load, make the snapshot path a required argument, and move the allowed transition matrix onto `BackfillOptions`. Satisfies **AC-D4**, **AC-D5**.
...
8. Run it for real in the same sitting and confirm the null sets and the frozen PEAK and FALLING classes. Satisfies **AC-D4**, **AC-D6**, **AC-D8a**, **AC-D9**.
