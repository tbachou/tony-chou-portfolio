...
- **AC-D8a**: the drift check is mechanical, not visual. Ingest and rescan stay ungated and a revision landing on an old `validTime` is visible to a hindcast row's reconstruction, so the write run queries `PipelineRun` for any `USGS_INGEST` or `USGS_RESCAN` that was **alive past the snapshot's `takenAt`**: started after it, finished after it, or never finished at all, looked for within a bounded window before the instant (an hour, three times the workflow's kill timeout). Matching on `startedAt` alone was the letter as first built and shipped, and the predeploy review caught its hole the same day: a run that starts just before the snapshot and commits revisions just after it straddles the instant invisibly. The write also **refuses when no snapshot exists at its path**, because a write that takes its own fresh snapshot anchors this check at its own start and skips the reviewed report entirely. Comparing two printed transition matrices by eye is kept only as a secondary human check, because a large multi group table read under time pressure is not a control.
...
| backfill | whether a rescan intervened | derived: any `PipelineRun` with job `USGS_INGEST` or `USGS_RESCAN` alive past the snapshot's `takenAt` (started after it, finished after it, or unfinished), per **AC-D8a** |
