...
- **AC-S4**: When the reading is stale **and** the pipeline is not completing runs, the warning says so. **Health is read from the `USGS_INGEST` job alone, never from the combined set the run panel displays**: the rescan step runs even when ingest fails and records `OK` with nothing to re-poll, so the newest combined row is a fresh success for as long as ingest is broken, and this criterion becomes unsatisfiable. Not completing means any of: the most recent ingest run recorded `FAILED` or `PARTIAL`; that run's own `startedAt` is older than the threshold; or no run exists at all. The last two matter because **a scheduler that stops entirely writes no new row**, so the most recent run stays the last successful one and its status stays `OK`. Reading status alone reports the worst failure this page has, a workflow GitHub disabled after sixty days of repository inactivity, as perfect health. When the reading is stale and the pipeline is merely late, the sentence is withheld, because late and broken are different findings.
...
- **AC-S7a**: The marker is perceivable without sight. A `title` on a role-less `<span>` is not an accessible name and is not announced, and `‡` is punctuation that a screen reader does not speak at default verbosity, so the per row marker as first built was silent to exactly the reader who cannot see the table. The glyph is `aria-hidden` and carries visually hidden text stating the marker and its legend, so the marked row reads as marked. This is the mode that matters: in the all stale case the note is a plain paragraph before the table and was never in question, while the per row case is the one where marked and unmarked rows sit together and the distinction carries the information. The `*` and `†` markers share the underlying pattern and are left alone; they qualify provenance, and this one qualifies whether a number about a river can be trusted.
...
- **AC-S8b**: The forecast read's window is derived from the longest horizon, not written as two days. A forecast is still about the future while `issuedAt + horizonHours > now`, so the set of live forecasts reaches back one full longest horizon; a window shorter than that hides rows **AC-S8** would have kept. At the two day window this child was built on, and a 72 hour longest horizon, that gap is 24 hours wide: an outage between 49 and 71 hours old leaves the last slot's 72 hour rows still targeting the future and outside the query, so the table empties and **AC-S9**'s note asserts that every forecast on record has elapsed while two live ones sit in the store. Both audit passes found this independently, one by sweeping outage ages and one by arithmetic. Deriving the window from `max(HORIZON_HOURS)` closes it and makes **AC-S9**'s claim true rather than merely careful: past one longest horizon, every forecast ever issued has necessarily elapsed. The widening cannot change a healthy render, because the newest claim per forecaster and horizon is what the table shows and the extra rows are older ones the same dedup discards. This is the same discipline as **AC-S1**: the relationship is what gets recorded, so a new horizon moves the window with it. One visible consequence, correct rather than incidental: a forecaster retired by clearing its active flag keeps its last slot on the page for up to one longest horizon beyond where it used to disappear, because those rows are still about the future and **AC-S8** is the rule that decides, not the window. They arrive marked, since anything the widening newly admits was issued at least two days ago and is therefore stale on its own clock.
...
| forecast table | how far back forecasts are read | `max(HORIZON_HOURS)` before `now`, so no row **AC-S8** would keep falls outside the query (**AC-S8b**) |
...
| `STALE_INGEST_NOTE` | The job that feeds this page is not running normally either. |
...
| `STALE_FORECAST_LEGEND` | Issued more than {hours} hours ago, or built on a river reading that is now more than {hours} hours old. Either way it was made without the river's current level, and may be well off. |
| `EVER_ISSUED_UNKNOWN_NOTE` | No current forecast is showing, and this page could not check whether one has ever been issued. That is the page failing to ask; it is not news about the river. |
...
That rule caught the legend a second time, and the third catch is why the clause now names a clock. **There are three triggers, not two.** Beyond a stale `issuedAt` (**AC-S5a**) and a stale input at issue time (**AC-S5**), the per row predicate ORs in `readingIsStale`, so a row is marked whenever the page is warning about the newest reading at all. Under that third trigger a forecast issued three hours ago from an input that was six hours old when it was issued gets marked, and both of the old clauses — "issued more than nine hours ago", "from a river reading that old" — are false as **AC-S5** defines them, because both measure age at `issuedAt`. Two audit passes found this independently. The second clause is therefore anchored to **now** rather than to issue time: a reading that is *now* older than the threshold. That is true under all three triggers. For the third, because the input reading is never newer than the newest reading, so a stale newest reading makes every input stale by the same clock. For the first two it holds by construction, since a forecast issued past the threshold, or from an input already past it at issue, has an input at least that old now.

**The one branch that argument does not reach is `inputReadingFor` returning null**, which **AC-S6a** deliberately admits, and it is worth writing down why the sentence survives it rather than leaving the gap to be rediscovered. A null input means no loaded observation satisfies both bounds, so there is no reading for the clause to describe. It cannot arise from a row the pipeline wrote: `issuePredictions` reads its history through the same `recordedAt <= issuedAt` knowability bound over the whole record, and skips a forecaster whose history is empty, so a persisted prediction implies a qualifying observation existed. Null therefore means only that the observation has fallen outside the page's thirty day display window, which makes the clause substantively true and merely unnameable. That is a dependency on `OBSERVATIONS_DEFAULT_WINDOW_DAYS` and on the predictor's own bound, not on the copy: shrink the window, or add a forecaster that can issue with no observation, and the sentence goes false with nothing failing.

**The per row marker's `title` is the legend, not a paraphrase of it.** It is built from `staleForecastLegend` rather than written inline. Its wording was corrected once while its location was not, which left the same string free to drift the next time the legend changed. A copy rule that lives in this table cannot police a string that does not read from the module the table governs.
...
- A predictor dead for sixty hours, with the last slot's 72 hour rows still targeting the future: the table renders them marked rather than claiming every forecast on record has elapsed. The fixture's prediction read must honour the `issuedFrom` it is given, because a mock that ignores the window cannot fail on a window bug, verifies **AC-S8b**.
- A row marked in the per row mode exposes the legend to a screen reader, and the glyph itself is hidden from one, verifies **AC-S7a**.
- The third trigger's sentence: a stale reading with a forecast issued three hours ago from an input that was six hours old at issue, where both of the old legend clauses are false and the amended one is true, verifies the Copy table's first rule.
...
- [ ] `isStale` fails toward silence on a timestamp in the future. It compares a signed age, so a `validTime` or a run's `startedAt` ahead of `now` is never past the threshold, and one future dated row turns off the reading warning, the ingest note and every forecast marker at once while the page prints "-180 min ago" beside the figure. Both audit passes found it. It is left alone deliberately rather than patched: the reachable cause is clock skew between machines (`startedAt` is stamped by a GitHub Actions runner, `now` by Render) and the honest fix needs a copy string for a timestamp that cannot be used, which this table does not have and which should not be invented in a hurry on a public safety surface. Decide the copy, then fix the predicate. The same change closes a latent `NaN` path, currently unreachable because `parse.ts` throws on an unparseable `dateTime` and both columns are non nullable.
- [ ] `relativeAge` rounds to the nearest hour while `isStale` is exact, so across a thirty minute band two readings both render "9 h ago" and only one carries a warning, and the warning's own age can read as though it has not cleared its own threshold. Cosmetic, and it only became visible when this child put that string inside a safety sentence. Flooring at this one call site would fix it; `relativeAge` has other callers, so it is a change to make deliberately.
- [ ] The run panel shows the newest of ingest and rescan while health is read from ingest alone (**AC-S4**), so during the outage that criterion exists for a reader can scroll from "the job that feeds this page is not running normally" to `last job: usgs rescan · ok · 1 h ago`. Neither statement is false and the split is what makes **AC-S4** satisfiable, but the pair invites a reader to discount the warning. Surfacing the last ingest age beside the run row would settle it; the panel is a four column grid and a fifth cell is a design decision, not a patch.
- [ ] The failed forecast read is the one failure path that withholds the redirect, while the failed reading read now carries it. The current rule is defensible, since a reader who cannot see the forecast table still has a current number on screen, but the two paths were decided at different times and should be decided once.
