...
- **AC-R5**: The backfill runs one request per calendar month per lead, each recording a `PipelineRun` with `job` of `OPEN_METEO_INGEST` and its `windowStart` and `windowEnd`. A resumed run skips any month already recorded `OK` for that lead. Re-running a completed month writes zero rows. How those rows reach the database is set by **AC-R16**, which is a separate concern from how they are fetched.
...
4. One month of one lead ingested end to end against the real archive, recording its `PipelineRun`. This is the thin thread; stop and look at the rows, and at the statement count, before going wider. Satisfies **AC-R5**, **AC-R14**, **AC-R16**.
