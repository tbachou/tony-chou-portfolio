...
- **AC-R5**: The backfill runs one request per calendar month per lead, each recording a `PipelineRun` with `job` of `OPEN_METEO_INGEST` and its `windowStart` and `windowEnd`. A resumed run skips any month already recorded `OK` for that lead. Re-running a completed month writes zero rows.
...
| backfill | which months remain | derived: months with no `PipelineRun` recorded `OK` for that lead |
...
3. `forecastsAsOf`, the latest known read on the four column key, plus the diff and write path above it: write only on a changed value, never update, treating an undefined and a null `tempC` as equal. Mirrors the observation ingest in shape, not in code, because `selectChangedReadings` keys on `validTime` alone. Satisfies **AC-R3**.
4. One month of one lead ingested end to end against the real archive, recording its `PipelineRun`. This is the thin thread; stop and look at the rows before going wider. Satisfies **AC-R5**, **AC-R14**.
5. Widen to all three leads and all months, chunked and resumable, skipping months already recorded `OK`. Satisfies **AC-R5**.
...
- Roughly seventy thousand new rows, which is the same order as the observation table and free at this scale.
