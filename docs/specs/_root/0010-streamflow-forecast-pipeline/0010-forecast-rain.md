...
- [ ] [verify.md](verify.md) records acceptance criteria coverage for the parent and the earlier children but for no **AC-R** at all. The resume key is cheap to check against the live store, by counting the `OK` `OPEN_METEO_INGEST` runs per lead and comparing against the months the archive covers, and belongs there when this child is verified.
