...
- [ ] One thing the jsdom suite structurally cannot see: vitest loads no CSS, so a Tailwind visibility class hides the credit with every test green. Confirmed on 2026-08-31: `hidden`, `sm:hidden`, `invisible`, `text-[0px]` and `text-transparent` all survived, while an inline `style={{display:'none'}}` was caught, which shows the suite would catch it if the rule were reachable. The realistic path is a `hidden md:block` added during a mobile pass, taking the credit off phones only. No cheap fix; recorded as a known bound of every jsdom suite in this workspace
