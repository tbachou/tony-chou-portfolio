...
- [x] `npm test --workspace=apps/web` → 89 green across 10 suites, 8 of those tests in the new `app/streamflow/DataSources` suite: the source link, the licence link, both asserted by `href`, `target` and `rel` on both, the USGS credit kept, the forecast rather than observed rain wording, the timezone passed in rather than baked in, and the two page wiring checks below → AC-R15
...
- [x] **That the page still renders the credit at all, which the component suite does not prove.** The pre deploy audit on 2026-08-31 found this hole and confirmed it by deleting the import and the JSX call from `page.tsx`: 87 tests passed, `tsc` exited 0, and lint was byte for byte identical to baseline, so the site could have gone out of licence with the whole gate green. Closed by `the dashboard page wiring` in `DataSources.spec.tsx`, which asserts the import and the render against the page source. Re mutated after the fix: deleting both lines fails 2 tests, deleting only the JSX call fails 1, and commenting the call out rather than deleting it also fails 1, which is what proves the comment stripping is real → AC-R15
- [x] The bound of that wiring guard, recorded rather than left implied. It reads the page source instead of rendering the page, because rendering `page.tsx` would mean mocking four Prisma methods, six package functions and the fetch driven client panels, tying a licence assertion to the whole data layer so it fails for unrelated reasons and gets deleted as flaky. It therefore catches deletion, which is the realistic edit, and does not catch a render that is present but unreachable, such as `{false && <DataSources />}` → AC-R15
