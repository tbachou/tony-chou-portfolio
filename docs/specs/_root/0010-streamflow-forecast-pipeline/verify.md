...
- [ ] Push the branch to main (predeploy gate first), set the flag back to `true`, and confirm the next slot issues at all three horizons with `intervalSeeded` true, the score job resumes, and ingest never stopped → AC-D8, AC-D11
