...
- [x] `npm test --workspace=apps/web` → 88 green across 10 suites, 7 of those tests in the new `app/streamflow/DataSources` suite: the source link, the licence link, both asserted by `href`, `target` and `rel` on both, the USGS credit kept, the forecast rather than observed rain wording, the timezone passed in rather than baked in, and the page render check below. CI runs this exact command in its `verify` job, which is the check the `main` ruleset requires, so the guard holds on `main` and not only locally → AC-R15
