...
- [x] `npm test --workspace=apps/streamflow` → 530 green across 40 suites, including 218 across the fourteen rain suites: `openmeteo/client` 11, `openmeteo/parse` 16, `asof/forecasts.repository` 24, `asof/forecast-as-of` 14, `forecast/rain` 18, `forecast/rain.repository` 16, `forecast/leakage` 9, `forecast/wetness` 11, `ingest/forecast-window` 36, `ingest/forecast-write` 8, `ingest/forecast-diff` 10, `ingest/ingest-forecasts` 12, `ingest/backfill-forecasts` 18, `ingest/live-forecasts` 15 → AC-R1, AC-R2, AC-R3, AC-R4, AC-R5, AC-R6, AC-R7, AC-R8, AC-R8a, AC-R9, AC-R10, AC-R11, AC-R12, AC-R13, AC-R14, AC-R16
...
- [x] Against a throwaway local Postgres with the migrations applied, insert a `weather_forecasts` row with `leadHours` of 0 → rejected by `weather_forecasts_lead_hours_check`. The parser half is covered by the suite above; this is the database half, and the whole point of AC-R2 is that the two catch different mistakes. Run 2026-08-31 on Postgres 16 with all eight migrations applied: `leadHours` of 0 and of 12 both rejected by name, `leadHours` of 24 accepted → AC-R2
- [x] The second check constraint, `weather_forecasts_issued_before_recorded`, added 2026-08-31 by migration `20260831143242` after the pre deploy audit measured the leak margin at **zero milliseconds**. The live window reaches to now plus the lead, so its last hour is issued at the instant the run starts; before this child that margin was at least a lead, 24 hours at the shortest, because `clampWindowTo` refused every future hour. A backward step of the wall clock between computing the window and stamping the rows then inverts the two axes, the row is written, and the run still records `OK` because nothing checked. Created with `migrate dev --create-only` and applied by `migrate dev`, as AC-R2 requires of the first one. Verified on a throwaway: `issuedAt` one second after `recordedAt` rejected by name, `issuedAt` exactly equal to `recordedAt` accepted (the boundary the live window's last hour actually produces), an ordinary row accepted. Preflighted read only against the live store first, which holds zero weather rows, so it cannot fail on deploy → AC-R4, AC-R9
- [x] Against a throwaway seeded with two months at two leads, run the backfill twice → the second pass fetches nothing and writes nothing. The suite proves the skip against a stubbed reader; this proves it against real rows and a real unique key. Run 2026-08-31 over June and July 2026 at leads 24 and 48: first pass 4 chunks run, 2,928 rows, all OK; second pass 4 skipped, 0 fetched, 0 written, store unchanged at 2,928 → AC-R3, AC-R5
...
- [x] Live window start, read off the store rather than the schedule: `starts each lead at the greatest validTime stored for it, less the overlap`, `asks for the whole gap after a missed run, without consulting a schedule`, and on the window itself `asks for the whole gap when a run was missed`, `starts at the live edge when the store holds nothing for the lead` → AC-R13
- [x] Live window end, the reason the job exists: `reaches one lead ahead of now, so a prediction issued now has a full window`, `ends one lead ahead of now, so the last hour was issued exactly now`, `reaches further ahead at a longer lead` → AC-R13
- [x] That the live window's far edge is the boundary and not a margin: `never reaches an hour whose nominal issue time is after now` on the window, and `writes no row whose nominal issue time is after the run` on the job. Both hold for all three leads → AC-R13, AC-R4
- [x] One run per lead, since a run carries a single lead: `records one OPEN_METEO_INGEST run per lead, each carrying its lead`, created already saying FAILED → AC-R13, AC-R5
- [x] That a live run is not a backfill chunk: `records the window it actually asked for, not a month`, `ignores a live run whose window happens to start on a month boundary`, `still runs the month a live run only appeared to cover` → AC-R13, AC-R5
- [x] Live run status, without the backfill's refusal: `is OK on a full window even though the window ends in the future`, `is PARTIAL when the service returned fewer hours than the window implies`, and on the rule `does not care whether the window has elapsed`. Plus the refusal the pre deploy audit added on 2026-08-31: `is PARTIAL on a window that implies no hours at all`, `gives an empty window the same answer on and off the hour`, `is PARTIAL on the window a collapsed live run produces`. A run that asked for nothing used to record `OK`, and whether it did depended on the sub hour position of the clock rather than on the data → AC-R13, AC-R14
- [x] That one bad lead does not cost the others, and that the failure is not merely counted: `runs the remaining leads after one fails, and counts the failure`, `says which lead failed and why, rather than only counting it`, `reports no failures on a clean cycle`, and `reports a failure that happened before the run row existed`. That last one is the case the run row cannot cover, since `ingestForecastWindow` upserts the gauge and creates the row before its own try block, so a failure in those first steps leaves nothing in the database to read the reason off. Found by the cross model review on 2026-08-31, not by the build. Both reason carrying tests fail if the reason is dropped again → AC-R13, the parent's AC-5
- [x] Live ingest cost: `costs a handful of statements per cycle, never one per hour`, `reads the stored edge once for every lead, not once per lead`, and on the read `issues one statement for every lead, not one per lead` → AC-R13, AC-R16
- [x] End to end against a throwaway Postgres with the migrations applied, 2026-08-31, the live store untouched. `npm run ingest:weather` on an empty store wrote 150 rows over three runs, one per lead, each `OK` and each carrying its own lead and window. Re-run a minute later: the window had narrowed to 3 hours per lead and wrote 0 rows, so the edge really is read back out of the store. Seeded with a month old edge at lead 24 only, the next run asked for 752 hours at that lead while leads 48 and 72 asked 50 and 74, which is gap recovery per lead and no schedule anywhere. Across the 23,786 rows the runs left behind: 0 with `issuedAt` after `recordedAt`, 0 where `issuedAt` was not `validTime` minus `leadHours`, 0 below the minimum lead, and the newest row at every lead sat exactly one lead ahead of the run → AC-R13, AC-R4, AC-R9
- [x] Live run cost and status, same session. A run writing 873 rows across three leads cost 23 Postgres statements, 3 of them inserts. A run over the January 2024 ramp returned 22,937 of 23,383 implied hours and recorded `PARTIAL` while the other two leads recorded `OK` in the same cycle, and its 22,937 rows were written as exactly 23 inserts, which is the 1,000 row batch size holding above one batch → AC-R13, AC-R14, AC-R16
- [x] That a live run cannot be mistaken for a backfill chunk, proven on real rows rather than a stub. Forced the collision: left lead 48 with a stored edge two hours into a month so the next live run recorded `OK` with `windowStart` exactly `2026-08-01T00:00:00`. `completedForecastChunks` returned only the one genuine July chunk. Keyed on `windowStart` alone, the same rows would have told the backfill August at lead 48 was finished → AC-R5, AC-R13
- [x] **And the case that guard does not close**, found by the pre deploy audit on 2026-08-31 and pinned by `can still be fooled when both ends land on a month exactly`. Both ends of a live window can align with a real month at once: lead 24, stored edge `2026-09-01T02:00:00.000Z`, run starting `2026-09-29T23:00:00.000Z` is exactly September and reads as a finished chunk. What makes it survivable is not the guard but the exact millisecond, since one millisecond either side breaks it, and the store must also be a month stale at that lead. Roughly one run in three and a half million, resting on the wall clock rarely landing on a round number. The docstring said this was closed and now says what it actually is. Telling the two kinds of run apart positively, by recording which job wrote the row, is the real fix and is a schema decision rather than a patch → AC-R5
- [x] Live probe against the real endpoint, 2026-08-30, no database touched. Built the live window for each lead from a store edge one cycle behind and fetched it: all three returned a full window, 8 of 8 hours, status OK, and the newest row's nominal `issuedAt` landed at or before the probe instant every time. So the far edge is reachable rather than merely intended → AC-R13
- [x] The same probe pushed six hours past the boundary, which is the adversarial half and the one that earns the rule. All three leads returned those hours, and six per lead carried a nominal `issuedAt` **after** the probe instant. The service answers a target it has no run for from the nearest run it does have, so a window one step wider would store rows claiming a lead they never had. The boundary is load bearing, not cosmetic → AC-R13, AC-R9
...
- [x] `npm test --workspace=apps/web` → 94 green across 10 suites, 13 of those tests in the new `app/streamflow/DataSources` suite. CI runs this exact command in its `verify` job, which is the check the `main` ruleset requires, so the guard holds on `main` and not only locally → AC-R15
- [x] That the suite can actually fail, proven by mutation on 2026-08-31 rather than assumed. Twenty mutations were run against the component and nineteen were caught, including swapped `href`s, `rel` weakened to `noopener`, `target` dropped, the text changed to `CC BY-NC 4.0` with the correct `href` kept, an `https` to `http` downgrade, a lookalike domain, and the licence unlinked to plain text. A credit is exactly the kind of thing a copy edit removes silently, so a test that cannot fail here is worth nothing → AC-R15
- [x] **That the page really renders the credit, which the component suite does not prove.** The pre deploy audit on 2026-08-31 found this hole and confirmed it by deleting the import and the JSX call from `page.tsx`: 87 tests passed, `tsc` exited 0, and lint was byte for byte identical to baseline, so the site could have gone out of licence with the whole gate green. Closed by `the dashboard page` in `DataSources.spec.tsx`, which renders the real page behind two module mocks and three panel stubs and asserts both links by `href` on the rendered output → AC-R15
- [x] **The first fix for that hole was wrong, and the record is worth more than the tidy version.** It asserted against the page SOURCE TEXT, and the adversarial re run took it apart the same day. Text could not tell a render from a mention, so it passed on an aliased import (`DataSources as _Legacy`) that rebound the JSX to a stub module with no credit in it, and passed again when the render was deleted and the only surviving `<DataSources` sat inside a string reading "restore the credit before launch". It also FAILED while the credit was correctly wired, because a `/*` inside an ordinary line comment opened a phantom block comment that ate the import, and that direction is the dangerous one: a guard that cries wolf is the guard someone deletes. Rendering was rejected first on the grounds that mocking the data layer would make it flaky; the adversarial pass showed that trade was priced wrong, since the mocks are six lines and the text guard did not guard → AC-R15
- [x] The render guard re mutated against every bypass that beat the text one, 2026-08-31, each applied to a full 643 line `page.tsx` rather than a truncated one: plain deletion of both lines caught, the aliased import onto a stub module caught, `<DataSources` surviving only inside a string caught, and `{false && <DataSources />}` caught, which the text guard had documented as a bound it could not close. The routine line comment holding `/*` that broke the text guard now passes 7 of 7, because nothing parses source any more. Baseline green before and after, and the suite also passes under `vitest run --root apps/web`, the invocation that made the text guard throw ENOENT → AC-R15
- [x] **The render guard was then broken four more ways by a second adversarial pass, and all four are fixed.** Two were cries wolf failures, which are the dangerous kind because they are what gets a test deleted: moving the footer into its own `async` server component, the ordinary Next 15 pattern, collapsed the whole tree and failed with a message identical to the credit having been deleted, and adding a second perfectly legitimate Open-Meteo link failed on `Found multiple elements`. Two were blind spots: the fixture returned an empty store, which is the opposite of production, so a data gated credit would have been proven in the wrong direction, and an unmocked model reached through `Promise.allSettled` was swallowed by `settled` into a `console.error` nothing asserted on, so the mocks could rot out of step with the page in silence → AC-R15
- [x] What those four fixes are, and the proof each works. The fixture is now production shaped (18,849 readings, a real last run, a real newest reading). The test asserts no `[streamflow]` error was swallowed, which turns silent mock drift into a failure: adding `prisma.prediction.count` to the `allSettled` list passed 7 of 7 before the fix and fails 1 after it. Links are matched by `href` rather than by accessible name, so the second Open-Meteo link now passes 7 of 7. And `getByRole('main')` is asserted first, so the two failures now read differently: a collapsed tree says `Unable to find an accessible element with the role "main"` while a genuinely deleted credit says `expected [ '#main-content' ] to include 'https://open-meteo.com/'` → AC-R15
...
- [x] **The safety line now sits beside the numbers, not only in the footer.** A public safety audit on 2026-08-31 measured roughly 3,400px between the forecast table and the footer block, about three and a half screens on a desktop and six to eight on a phone, and pointed out that `/streamflow` is indexed and is the page most likely to be reached by a direct link. A reader who acts on a forecast plausibly never scrolled to the disclaimer. It is now repeated beside the latest reading and under the forecast table, from a single exported `NOT_A_FLOOD_FORECAST` constant so three copies cannot drift apart. Pinned by `repeats the safety line beside the numbers, not only in the footer`, which asserts three occurrences on the rendered page and fails at two when either repetition is removed
- [x] **The disclaimer now says what to consult instead.** It previously said only what not to trust, and `/streamflow` carried no authoritative link at all: its only two links were `open-meteo.com` and `creativecommons.org`, both added by this branch, while both sibling pages already linked USGS. It now points at the USGS gauge, NOAA's National Water Prediction Service, and local emergency services. Both URLs were checked for a 200 before shipping, since a dead link on a safety line is worse than no link. Pinned by `points at an authority instead of only saying what not to trust`
- [x] **The `/streamflow` dashboard now says it is not a flood forecast, which it never did.** The pre deploy audit found that the one page showing live predictions for a real river carried no such line, while `/projects/streamflow` and the walkthrough have carried one since they shipped; confirmed absent on `origin/main` too, so it was a standing gap rather than something this branch caused. The wording is copied from those two rather than rewritten, so the three pages say one thing. It leads the block rather than trailing it. Pinned by `says this is not a flood forecast, on the page that shows forecasts`, and mutation checked: deleting the paragraph fails 1, and softening `not a flood forecast` to `not always accurate` also fails 1, so the load bearing phrase cannot be quietly weakened
- [x] Attribution links no longer read the decorative arrow aloud. Measured in a real browser on 2026-08-31 by stripping `aria-hidden` subtrees the way the accessible name algorithm does: the two credit links compute to `Open-Meteo` and `CC BY 4.0`, where they previously computed to `Open-Meteo ↗` and `CC BY 4.0 ↗`, while the arrow still renders for sighted readers. Applied to the whole streamflow surface for consistency, so the walkthrough's reference list and the case study's gauge link changed with it. `/beta`'s two arrows were deliberately left alone, being a separate feature and feature frozen → AC-R15
- [x] The walkthrough now links CC BY 4.0 rather than naming it in prose, since attribution under that licence means pointing at it. Verified rendering at `https://creativecommons.org/licenses/by/4.0/`
- [x] `replaceAll` on both underscore substitutions in `page.tsx`. The job label one was latent rather than live: `OPEN_METEO_INGEST` renders `open meteo_ingest` under `replace`, and this child shipped the job that writes that value, so widening the dashboard's job filter would have exposed it
- [x] The credit is legible on a phone, and does not push the page sideways. Measured in a real browser on 2026-08-31 against compiled Tailwind and the actual `terminal.css`, through the real element chain: at 375px the document `scrollWidth` equals its `clientWidth`, so no horizontal overflow, with the two `inline-block` links at 94px and 86px and both wrapping cleanly onto their own lines. Still no overflow at 320px. The first width that does overflow is 315px at 200 percent text zoom, which is past WCAG 1.4.10's 320px bar and is driven by the panel's own padding rather than by these links → AC-R15
- [x] **A third adversarial pass broke the tests again, and the pattern is the finding.** Two HIGH false passes. The accessibility test asserted that a `span[aria-hidden]` holding the arrow existed, which is a PROXY for the property rather than the property: markup carrying a bare arrow AND the hidden span passed while computing to the name `Open-Meteo ↗`. And the `replaceAll` fix had no guard at all, so reverting both call sites to `replace` left the whole suite green. Two MEDIUM false failures came with them: adding a correct `aria-label`, and adding any arrow free link to the block, each failed a test while the code was right. The pattern across all three rounds is one mistake repeated: pinning something adjacent to the property (page source instead of a render, a hidden span instead of an accessible name) rather than the property itself
- [x] The fixes, each mutation checked on 2026-08-31. Accessible names are now asserted as exact strings through testing library's own name computation, so the bare arrow leak fails 1; the job label is asserted on the rendered page with the fixture set to `OPEN_METEO_INGEST`, the only `PipelineJob` member with two underscores, so reverting `replaceAll` fails 1; the credit links are selected by `href` rather than by taking every link, so the `aria-label` and extra link mutations now pass 13 of 13 as they should; and the disclaimer's leading position, previously claimed in prose and unasserted, fails 1 when moved to the end of the block
- [ ] **The one bound that stayed open, found independently by both adversarial passes.** vitest loads no CSS, so a Tailwind visibility class hides the credit with every test green. Confirmed on 2026-08-31 on the component (`hidden`, `sm:hidden`, `invisible`, `text-[0px]`, `text-transparent` all survived) and again on the page, where wrapping the render in `<div className="hidden">` passes 7 of 7. An inline `style={{display:'none'}}` **is** caught, which shows the suite would catch it if the rule were reachable, so this is precisely a blind spot for the mechanism this Tailwind codebase actually uses. The realistic path is a `hidden md:block` added during a mobile pass, taking the credit off phones only. It is structural rather than a defect in this suite: no jsdom test in this workspace can see CSS, so closing it needs a real browser check in CI, which is a bigger decision than one attribution. Accepted as a known bound and left open deliberately
