...
- [ ] **Does "daily" survive?** The daily cadence is what creates the sourcing treadmill, and retention is not something a portfolio actually needs. The portfolio value is demonstrating vision reasoning with a visible explanation, which a fixed set of five to ten problems delivers just as well as an endless one, from photos the owner already has the right to use (`own_photo`, the source of the one photo in the pool today). Worth deciding before R7 and R8 rather than after, because R7's share button and R8's "grow the pool to ten or more" both assume the daily framing. A related hedge that needs no third party: show aggregate player guesses as a consensus grade beside the gym grade, which turns grade disagreement from a complaint into content and uses the histogram already stored.
