...
- **AC-20** (added 2026-08-21, sharpened 2026-08-22): once a `GradeDay` row exists, the image, the true grade and the analysis for that date all come from the photo the row pinned, even if that photo is later deactivated. The daily cycle decides only which photo a date gets when no row exists yet, so uploading or retiring a photo mid day cannot change the answer under visitors already playing.
- **AC-21** (added 2026-08-22): **serving the day pins the day.** `GET /grade/today` creates the `GradeDay` row for that UTC date if it does not exist, by the same atomic insert the guess path uses. A visitor is therefore never graded against a photo that was not the one on their screen, whatever happens to the pool between their view and their guess. The insert is idempotent and keyed by the date, so repeated views, prefetches and crawlers add no rows beyond the first, and a concurrent view and guess resolve to whichever insert won.
...
| GET /grade/today | photo choice | if a `GradeDay` row exists for the date, its `photoId`, always (AC-20). Otherwise days since epoch modulo pool size, over ACTIVE photo ids sorted lexically, with `unlicensed_test` rows excluded whenever the game is enabled, **and that choice is then written as the day's row before responding** (AC-21). Note the sort gives determinism ACROSS INSTANCES, not stability across uploads: changing the pool size reshuffles which photo every FUTURE date lands on, which is harmless because nobody sees the schedule, and AC-20 plus AC-21 are what protect the current day |
| GET /grade/today | side effect | one idempotent `GradeDay` insert, keyed by the UTC date (AC-21). This read endpoint writes on purpose: without it the day is unpinned until somebody guesses, and the pool can move underneath a visitor between their view and their guess |
...
- **The row is created by the first view, not the first guess (AC-21).** AC-20 protects a day only once its row exists, so whoever creates that row decides how wide the unprotected window is. Creating it on the first guess leaves the whole period between a visitor's view and their guess open, which on a low traffic site is most of the day, and the admin upload page exists precisely so photos get added at arbitrary times. The concrete failure: a visitor sees photo A at 09:00, a photo is uploaded at 09:05 changing `pool.length`, the visitor guesses at 09:10 and is scored against photo B's `trueGrade` for an image that was never on their screen. Pinning on the read closes it, and costs at most one row per UTC date because the date is the primary key.
...
R6a. Pin the day on read: `getToday` creates the `GradeDay` row through the same atomic insert `submitGuess` uses, so a view settles the date. The guess path keeps its own `ensureDayRow`, which becomes a no op whenever a view already ran. Correct the `photo-pool.ts` comment that currently claims "the current day is protected by AC-20 instead", which was only true after the first guess. Tests: a view creates the row; a second view creates nothing further; a pool change between view and guess does not move the answer; a guess with no prior view still works, satisfies **AC-21**, and completes **AC-20**.
...
- `GET /grade/today` writes (AC-21). A read endpoint with a side effect is a smell worth naming rather than hiding: it means a crawler, a link prefetch or an uptime check can settle which photo a date gets. The insert is idempotent and keyed by the date, so the blast radius is one row and never a duplicate, but the first thing to touch the endpoint on a given UTC day decides that day. In practice that is what "the day is fixed" means, and the alternative was leaving visitors gradeable against a photo they never saw.
- Once a day is pinned by a view, an upload cannot take effect until the next UTC day. Under the previous behaviour a same day upload could still land if nobody had guessed yet. That flexibility is what AC-20 exists to remove, so losing it is the point, but it does mean the pool has to be curated a day ahead rather than in the moment.
- `plays: 0` rows become normal. Any later reading of `GradeDay` row count as an engagement number would be wrong; `plays` is the engagement number.
...
- [ ] **Where the pool comes from is now the binding constraint, and it may deserve its own decision.** Everything R1 to R6 built is infrastructure for getting legally usable photos of real routes with a trustworthy grade attached, and R8 still asks the owner to shoot ten. Under research on 2026-08-22: whether standardized training boards (fixed angle, fixed hold positions, crowdsourced grades) could supply both imagery and a defensible ground truth, and whether generated imagery could ever carry a real grade rather than an asserted one. Note the shape of the problem before deciding: this game's value rests on `trueGrade` being a fact somebody set and climbed, not a label typed into a prompt. `GradePhotoSource` already anticipates three legitimate sourcing routes (`own_photo`, `permission_given`, `licensed`), so a new source kind would be a schema change and a new spec, not a tweak here.
