...
**Implementation skills**: `nestjs-best-practices` (`kadajett/agent-nestjs-skills`) · `javascript-typescript-jest` (`github/awesome-copilot`). Both are registry skills, so under spec 0014 they are installed once per machine into `~/.claude/skills/` and are not in this repo.

The `upstash` skill was installed for the first build slice and then removed from the repo, which is worth recording because the reasoning generalises. It earned its place twice, catching that a text upsert uses `/upsert-data` rather than `/upsert` and that a query needs `includeData` alongside `includeMetadata`. Both facts are now permanently captured in `vector-store.ts` and in this spec, so the skill had already delivered everything it was going to.
...
| Embedding model | **the hosted OpenAI option**, `openai/text-embedding-3-small` | The dashboard offers a binary here, hosted OpenAI or **custom**. This must be the hosted one. Custom means the index holds no embedding model, so it accepts only vectors and not text, which means this project would have to call an embedding API itself, hold that credential, and maintain that integration. That is the exact second provider this design exists to avoid, and picking custom silently undoes it |
...
- [x] Set the AC-5 similarity threshold from AC-14's golden cases once they exist, measuring precision and recall across candidate values. **Done 2026-09-01: 0.68**, calibrated against twenty labelled queries (ten the corpus answers, ten it does not), replacing the 0.62 placeholder. It keeps 10/10 positives and rejects 7/10 negatives; the two populations overlap, so no value separates them and none was expected to. The table and the reasoning live above `MINIMUM_SIMILARITY` in `retrieval/vector-store.ts`. **The calibration is now re runnable** — `npm run sweep:threshold` re measures it, which matters because the original was done against the 607 chunk index and a re embed repartitions the corpus.
...
- [ ] **Decide what to do about the writeup treadmill, found closing phase three.** `docs/evals/interview` is in the corpus, so publishing a phase writeup changes `corpusHash`, and AC-11 then declares the next run not comparable to the baseline that writeup reports. Closing this phase did exactly that: the baseline was measured against `2a0b9739…` and the corpus is now `6f842a00…`, so phase four starts with no comparable baseline no matter what it changes. It is not a bug — the corpus IS part of the system under test, and retrieval quality genuinely depends on it — but it means every phase pays for a re baseline it did not cause. The options are to exclude `docs/evals/interview` from the corpus (losing the writeups as retrievable material, which is some of the best evidence of how Tony works), to re baseline after the writeup lands rather than before, or to let comparability break and say so. Not decided here.
