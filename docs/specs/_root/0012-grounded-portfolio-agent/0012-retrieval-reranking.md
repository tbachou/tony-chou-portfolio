...
**Date**: 2026-09-21
**Updated**: 2026-09-24 (the measurement design, after the code shipped `off` in #111 and the first full eval ran)
...
- **AC-13**: `scripts/threshold-sweep.queries.ts` gains two labelled sets of negatives, each query expecting no document. **Story shaped**: questions of the kind the interviewer actually asks about Tony's employment stories, which this corpus cannot answer. **Clinically adjacent**: questions near Beta or near Tony's occupational therapy background that are not about Beta's engineering; for these, a Beta document returned by either arm is a failure even when the other checks pass. Tony reviews the labels before they gate anything.
- **AC-14**: Every eval result records its rerank arm in its metadata, plus `RERANK_MODEL_ID` when the arm is not `off`, plus a per case count of rerank fall backs. The publish loader never compares two runs whose arms differ, the same way it already refuses a corpus hash mismatch; a run with no arm recorded predates phase six and counts as `off`. An `enforce` run in which any search fell back is recorded, but is not eligible as a phase entry, because it measured a mix of two arms.
...
- An eval result always names its rerank arm, and two results with different arms are never compared.
...
- The publish loader refuses to compare runs with different arms, and reads a run with no arm as `off`, verifies **AC-14**.
- An `enforce` run with any fall back is recorded but is not eligible as a phase entry, verifies **AC-14**.
- The sweep prints the negatives bar, the Beta check and the superset check, and the probe sets are present and labelled, verifies **AC-11**, **AC-13**.
...
5. **Eval bar**, objective, at one commit: two full runs with `--rerank off` and two with `--rerank enforce`. Each pair's spread is its noise band, as spec 0011 AC-9 defines it (the per dimension spread between two identical full runs). `enforce` passes only if, on every dimension, its mean is no lower than `off`'s mean minus the larger of the two bands, and neither `enforce` run fell back on any search. This is the step that decides whether the grounding drop seen on 2026-09-24 belongs to the reranker.
6. On passing both bars, set `enforce` and publish the phase entry with its delta against the `off` arm at the same commit. Below either bar, the phase stops at `shadow` or `off`, and this spec records why. Per the premise note, that is a legitimate outcome.
