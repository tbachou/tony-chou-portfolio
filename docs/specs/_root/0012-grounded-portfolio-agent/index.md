...
- [0012-search-portfolio-retrieval.md](0012-search-portfolio-retrieval.md): phase three, decided and specced. A `searchKnowledge` tool the Tony persona calls on demand, over an Upstash Vector index of Tony's own committed specs, findings and eval writeups, with the corpus hashed and recorded so a score cannot depend on an index nobody can reconstruct. **Correction to what this line used to say.** It scoped the corpus as documents from the portfolio repo, Panel, Carryover and Streamflow, and pointed the phase at the [embellishment finding](findings/2026-08-31-grounding-embellishment.md). The design run found those do not meet: every eval story is employment work (Mailchimp, Product Forge, Topstep, Tensure, Fugue) and none of it lives in a repo Tony owns, so retrieval over his own documents cannot ground those stories and cannot move those two cases. The finding's cheaper remedy, a prompt rule against unsourced technical rationale, is the right fix for that and is tracked separately. What phase three actually adds is the capability the story fixtures never had: answering how Tony works, from the decisions and measurements committed in this repo.
...
- [0012-retrieval-reranking.md](0012-retrieval-reranking.md): phase six, decided and specced. Puts a judgement where `MINIMUM_SIMILARITY = 0.68` sits today. Retrieval widens to ten candidates behind a low floor, a System One model scores each one for relevance to the interviewer's question plus the persona's search query, and the best three go to the persona. Ships `off`, then `shadow` deciding nothing, and starts deciding only when it clears both a sweep bar over the twenty labelled queries `threshold-sweep.ts` carries (plus new probe sets) and an `off` versus `enforce` eval comparison at one commit. Fails open to today's behaviour. **Shipped `off` in #111 (2026-09-24)**; its measurement design was updated and built the same day in #112. **Why a second provider here and not on the credential check**: spec 0013 parked this model because a tie was not enough to justify it on a path that fails closed and protects a regulated healthcare credential. Retrieval is the opposite trade, where the cost of being wrong is a weaker answer.
...
- [x] Phase three child: `searchPortfolio` retrieval, [0012-search-portfolio-retrieval.md](0012-search-portfolio-retrieval.md). Upstash Vector, a `searchKnowledge` tool on the Tony persona, corpus of Tony's own committed decision documents.
