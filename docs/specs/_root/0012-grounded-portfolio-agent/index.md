...
- [0012-retrieval-reranking.md](0012-retrieval-reranking.md): phase six, decided and specced. Puts a judgement where `MINIMUM_SIMILARITY = 0.68` sits today. Retrieval widens to ten candidates behind a low floor, a System One model scores each one for relevance to the interviewer's question plus the persona's search query, and the best three go to the persona. Ships in shadow mode deciding nothing, and starts deciding only when it beats the current number on the twenty one labelled queries `threshold-sweep.ts` already carries. Fails open to today's behaviour. **Why a second provider here and not on the credential check**: spec 0013 parked this model because a tie was not enough to justify it on a path that fails closed and protects a regulated healthcare credential. Retrieval is the opposite trade, where the cost of being wrong is a weaker answer.
