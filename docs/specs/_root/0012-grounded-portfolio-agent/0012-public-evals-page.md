...
- **AC-13** (amended 2026-08-31, see below): This phase records no eval measurement of its own, and that is stated rather than left silent: a short writeup `docs/evals/interview/0012-phase-two-public-evals-page.md` is committed saying no run was made, why the phase cannot move the scores, and which course principles were applied and skipped. Its manifest entry sets `measured: false`. The umbrella's AC-1 exemption is noted in the umbrella `index.md`. The PR carries the `skip-evals` label so that later pushes do not spend budget re running a suite this phase cannot move.

  _Amendment rationale._ This originally said the phase "changes no model facing code" and that its PR "carries the `skip-evals` label", and both drifted from what the branch became. The branch now touches `apps/api/scripts/interview-eval/**` and `apps/api/src/modules/conversation/eval/**`, which are watched paths, because the phase grew a preflight guard and a dirty tree classifier. Neither can move a score, one runs before any model call and the other is a list of pathspecs, so the conclusion holds; the reasoning does not, and is corrected here. The label also cannot be present for the `opened` event: GitHub's create pull request API takes no labels, so every tool including `gh pr create --label` creates first and labels second. The first capped run therefore happens, and is worth having: after three rounds of guard surgery a single eight case run is cheap evidence that the harness still executes end to end. The label suppresses every run after it.
