...
- **AC-15**: The baseline the page publishes is itself clean. Before the page ships, one full eval run is made on a committed tree and accepted as the baseline, replacing the current `baseline.json`, which was measured from a working tree with uncommitted changes (`meta.gitDirty` true at `0d048e0`). The page's no dirty runs rule therefore holds for its own reference point. This run is a re baseline, not a phase measurement, and is recorded as such in the baseline history with its reason.
...
| the current baseline | the reference every delta is measured against | `baseline.json`, replaced by the clean re baseline run required by AC-15 |
...
- A run that is not named in `publishedRuns` does not appear, whatever exists in `results/`. Dirty runs (`meta.gitDirty` true) must never be published, and the build rejects a manifest entry whose results file has `gitDirty` true. The same rule binds the baseline itself (AC-15), which is why the current dirty baseline is replaced before this page ships.
...
0. Re baseline clean: run the full suite on a committed tree, accept the result as the new `baseline.json`, regenerate `scoreboard.md`, and record the move in the baseline history with its reason (the previous baseline was measured from a dirty tree). Do this first, because every later task reads the baseline, satisfies **AC-15**.
