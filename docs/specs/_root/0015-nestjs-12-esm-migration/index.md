...
- **AC-13**: no relative import specifier in `apps/api/src` or `apps/api/scripts` is extensionless. `grep -rE --exclude-dir=generated "from '\.[^']*'" apps/api/src apps/api/scripts | grep -v "\.js'" | grep -v controller-scan.spec.ts` returns nothing. Both filters are load bearing and neither hides a real miss. Without `--exclude-dir` the grep matches a JSDoc comment inside the generated Prisma client, which `postinstall` rewrites on every build. The three remaining matches in `controller-scan.spec.ts` are fixture source inside template literals — input strings fed to the scanner under test, not imports this file performs — and they stay in the pre-ESM form on purpose, since the scanner must keep recognising that form.
- **AC-14**: `madge --circular --extensions js apps/api/dist` reports no cycle on the **built** output. CommonJS tolerates import cycles by returning a partly populated module; ESM throws `ReferenceError: Cannot access 'X' before initialization` at boot instead. Checking `src` was the original wording and cannot pass: it scans the generated Prisma client (16 of the 17 cycles it reports) and it counts `import type` edges the compiler erases, which is what the one cycle in our own code turned out to be — `grade.service` imports a value from `grade-analysis.service`, which imports back with `import type`. The built graph is what Node actually loads, and it has no cycle across 170 files.
...
4. Append `.js` to all 365 extensionless relative import specifiers in `apps/api/src` and `apps/api/scripts`, by codemod, **while the package is still CommonJS**. `nodenext` accepts `.js` specifiers under CommonJS, so this is behaviour preserving for `tsc` and for the swc build, and isolates several hundred edits into their own reviewable commit. It is **not** behaviour preserving for the runner or for the spec 0016 scanner, and the claim that it "keeps the suite green" was wrong on both counts: ts-jest cannot resolve `./foo.js` against `foo.ts`, so 48 of 53 suites failed until a `moduleNameMapper` was added (removed again in task 7, since Vitest resolves it natively); and the controller scanner identified the validation pipe by module path suffix, so eight real routes silently reported `validated: false` and would have published a false "validates nothing" claim into `docs/api/`. Both are the failure class this migration keeps producing — green typecheck, green build, wrong artifact. Satisfies **AC-13**.
