...
- **AC-6**: `rm -rf node_modules package-lock.json && npm install` exits 0 with no ERESOLVE and without `--legacy-peer-deps`, and the resulting lockfile diff changes no package outside the NestJS, multer, and expo set. `legacy-peer-deps` in `.npmrc` is not an acceptable outcome here: `render.yaml` runs `npm install`, not `npm ci`, so the setting would silently apply in production and disable peer checking for every future install.
...
- **AC-10**: TypeScript 6 lands first, in its own pull request, with all four workspaces typechecking clean and the full root suite green.
- **AC-11**: the ESM build is proven to boot on a Render branch or preview deploy before the change reaches `main`.
...
**What does not change**: Prisma, the Prisma Postgres database, Render hosting, the Anthropic SDK, the AI provider split, every route, every schema, and the rate limit values. Spec 0001 stays Accepted; only its module format line is revised by this spec.

**Known migration surface, measured rather than estimated:**

- **253 relative import specifiers across 109 files carry no file extension.** This is the largest single work item and the one most easily underestimated. `apps/api/tsconfig.json` sets `module: nodenext`, so the moment `"type": "module"` lands each one is both a TS2835 typecheck error and an unresolved specifier at runtime. swc does not rewrite extensions. The generated Prisma client already uses `.js` specifiers and is excluded from that count.
...
**Prerequisite environment (blocks step 11, so set it up early):**

2. Create a second Render web service deployed from the migration branch, on the free plan, with its own throwaway database, `GRADE_GAME_ENABLED=false`, `BETA_OUTPUT_GUARD_MODE=shadow`, and the daily caps set to small values. It must not share the production `DATABASE_URL` (it would write real rows) or spend meaningfully against the `ANTHROPIC_API_KEY` ceiling. Supports **AC-11**.

**Migration pull request:**

3. Fix the optional peer chain that makes a clean resolve fail: `@react-three/fiber` declares `expo` as an optional peer, npm installs it, and its React range collides with React 19. Prefer a narrow root `overrides` entry removing that optional peer from resolution. Do not reach for `legacy-peer-deps`: `render.yaml` runs `npm install`, so it would apply in production too. Satisfies **AC-6**.
4. Append `.js` to all 253 extensionless relative import specifiers in `apps/api/src` and `apps/api/scripts`, by codemod, **while the package is still CommonJS**. `nodenext` accepts `.js` specifiers under CommonJS, so this is a behaviour preserving change that keeps the suite green and isolates several hundred edits into their own reviewable commit. Satisfies **AC-13**.
...
10. Upgrade `@nestjs/*` to 12.0.3, `@nestjs/throttler` to 6.7.0, and `@thallesp/nestjs-better-auth` to 2.8.0. Satisfies **AC-1**.
...
13. Deploy to the branch service from step 2 and verify boot, prompt loading, SSE streaming, and one shared schema validated request on real linux x64. Satisfies **AC-8**, **AC-9**, **AC-11**, **AC-15**.
...
3. Render branch deploy verification, then the pre deploy gate, then merge.
...
- The Render environment is linux x64 and the local machine is arm64 macOS. `apps/api/AGENTS.md` already records that a clean install on Render is not implied by one working locally, which is why step 9 exists.
